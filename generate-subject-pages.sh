...
for subject_dir in "$PDFS_DIR"/*/; do
  ...
  BG_CSS="body{background:transparent !important;}body::before{content:'';position:fixed;inset:0;z-index:-2;background-position:center;background-size:cover;background-repeat:no-repeat;filter:blur(3px);transform:scale(1.04);}html[data-theme=\"light\"] body::before{background-image:url('${SITE_URL}/assets/${slug}-bg-l.jpg');}html[data-theme=\"dark\"] body::before{background-image:url('${SITE_URL}/assets/${slug}-bg-d.jpg');}body::after{content:'';position:fixed;inset:0;z-index:-1;}html[data-theme=\"light\"] body::after{background:rgba(255,255,255,0.38);}html[data-theme=\"dark\"] body::after{background:rgba(7,8,18,0.74);}html[data-theme=\"light\"]{--surface:rgba(255,255,255,0.72);}html[data-theme=\"dark\"]{--surface:rgba(22,24,42,0.66);--line:rgba(255,255,255,0.14);}header{-webkit-backdrop-filter:blur(14px);backdrop-filter:blur(14px);}.pdf-item,.cat-card{-webkit-backdrop-filter:blur(18px);backdrop-filter:blur(18px);}"
